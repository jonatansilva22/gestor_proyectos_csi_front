...
import {
  ...
  notifyError,
  notifyInfo,
} from "../../../components/common/ToastNotify";
...
const ProfilePage: React.FC = () => {
  ...
  const backButtonTouch = useTouchButton(() => navigate(-1), {
    hapticFeedback: true,
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      name in passwordErrors &&
      passwordErrors[name as keyof typeof passwordErrors]
    ) {
    ...
    if (name === "username") {
      ...
      if (value.length > 0 && !usernameRegex.test(value)) {
        setProfileErrors((prev) => ({
          ...prev,
          username:
            "Formato inválido: 3-50 caracteres, solo letras, números, puntos, guiones y guiones bajos",
        }));
      } else {
        setProfileErrors((prev) => ({ ...prev, username: "" }));
      }
    } else if (
      name in profileErrors &&
      profileErrors[name as keyof typeof profileErrors]
    ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (updateData.username)
        lines.push(
          <li key="un">
            Nombre de Usuario: {originalUsername} &rarr; {updateData.username}
          </li>
        );
      if (updateData.first_name)
        lines.push(
          <li key="fn">
            Nombre: {originalFirstName} &rarr; {updateData.first_name}
          </li>
        );
      if (updateData.last_name)
        lines.push(
          <li key="ln">
            Apellido: {originalLastName} &rarr; {updateData.last_name}
          </li>
        );
      if (updateData.photo)
        lines.push(<li key="ph">Se actualizará la foto de perfil</li>);
      if (hasPasswordChange)
        lines.push(<li key="pw">Se cambiará la contraseña</li>);
    ...
    try {
      ...
      if (error.validationErrors) {
        ...
        Object.keys(error.validationErrors).forEach((field) => {
          if (
            ...
            setPasswordErrors((prev) => ({
              ...prev,
              [field === "current_password" ? "currentPassword" : field]:
                error.validationErrors[field],
            }));
            const msg = Array.isArray(error.validationErrors[field])
              ? error.validationErrors[field][0]
              : error.validationErrors[field];
            if (msg) notifyError(String(msg));
          } else if (field === "username") {
            setProfileErrors((prev) => ({
              ...prev,
              username: error.validationErrors[field],
            }));
            const msg = Array.isArray(error.validationErrors[field])
              ? error.validationErrors[field][0]
              : error.validationErrors[field];
            ...
            setProfileErrors((prev) => ({
              ...prev,
              firstName: error.validationErrors[field],
            }));
            const msg = Array.isArray(error.validationErrors[field])
              ? error.validationErrors[field][0]
              : error.validationErrors[field];
            ...
            setProfileErrors((prev) => ({
              ...prev,
              lastName: error.validationErrors[field],
            }));
            const msg = Array.isArray(error.validationErrors[field])
              ? error.validationErrors[field][0]
              : error.validationErrors[field];
        ...
        if (errorData.username) {
          ...
          const msg = Array.isArray(errorData.username)
            ? errorData.username[0]
            : errorData.username;
          if (msg) notifyError(String(msg));
        }
        if (errorData.first_name) {
          ...
          const msg = Array.isArray(errorData.first_name)
            ? errorData.first_name[0]
            : errorData.first_name;
          if (msg) notifyError(String(msg));
        }
        if (errorData.last_name) {
          ...
          const msg = Array.isArray(errorData.last_name)
            ? errorData.last_name[0]
            : errorData.last_name;
          if (msg) notifyError(String(msg));
        }
  ...
  return (
    <HeaderSidebarLayout headerTitle="Perfil de Usuario">
      ...
      <SuccessModal
        ...
        message={
          <div>Se guardaron correctamente los cambios de tu perfil.</div>
        }
      />
      <div
        className={`max-w-4xl mx-auto p-6 min-h-screen transition-colors ${
        ...
        <div className="bg-gradient-to-br from-purple-600 via-purple-700 to-purple-800 rounded-3xl p-8 mb-8 relative overflow-hidden">
          ...
          <div className="relative z-10 flex items-center gap-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-purple-100 text-lg opacity-90 mb-4">
                {user?.email}
              </p>
          ...
          <div className="p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex-1">
                ...
                >
                  Cambiar tu nombre de usuario puede afectar tu forma de iniciar
                  sesión
                </p>
              </div>
              ...
              <div className="flex-1">
                ...
                >
                  El correo electrónico no se puede modificar por razones de
                  seguridad
                </p>
              </div>
