...
export default function Permissions() {
  ...
  const handleDeleteClick = async (student: Student) => {
    if (
      window.confirm(`¿Estás seguro de que quieres eliminar a ${student.name}?`)
    ) {
      try {
  ...
  const handleCancel = () => {
    if (hasChanges) {
      if (
        ...
        loadStudents(); // Reload original data
        setHasChanges(false);
      }
    } else {
      navigate("/dashboard");
      ...
      <main className="flex-1 px-4 py-6 md:px-[53px] md:py-8">
        ...
        <div className="flex justify-center items-center space-x-6 mt-12">
          <button
            ...
            className={`font-inter text-base font-normal ${darkMode ? "text-white hover:text-gray-300" : "text-black hover:text-gray-700"} transition-colors duration-200`}
          >
            Cancelar
          </button>
