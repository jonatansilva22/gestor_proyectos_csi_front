  ...
  return (
    <div className="flex flex-col gap-2 w-full min-w-0" ref={selectRef}>
      ...
      <div className="relative">
        ...
        {isOpen && !disabled && (
          <div className={`absolute top-full left-0 right-0 mt-1 py-1 rounded-lg shadow-xl border z-50 max-h-60 overflow-y-auto ${darkMode ? 'bg-[#3A2B5A] border-purple-700/30' : 'bg-white border-gray-300'} ${
            darkMode 
            ...
            {options.length > 0 ? (
              options.map((option) => (
                <button
                  ...
                  className={`w-full px-4 py-3 sm:py-2.5 text-left text-sm sm:text-base transition-colors duration-200 focus:outline-none ${
                    value === option.value
                      ? (darkMode ? 'bg-purple-600/20 text-purple-300' : 'bg-purple-600/20 text-purple-600')
                      : (darkMode ? 'text-white hover:bg-purple-700/20' : 'text-gray-900 hover:bg-gray-50')
                  }`}
                  role="option"
